...
///
/// Must be constructed from within a multi-threaded tokio runtime context with at least two
/// worker threads — the [`Handle`] of that runtime is captured at construction time and reused by
/// the synchronous [`KVStoreSync`] and [`PaginatedKVStoreSync`] impls so that those calls always
/// reach the same runtime that hosts the underlying tokio-postgres connection drivers.
///
...
pub struct PostgresStore {
	...
	// Handle to the runtime captured at construction time. Sync ops `block_on` on this handle so
	// they drive the connection drivers spawned during `new`, regardless of which runtime the
...
impl PostgresStore {
	...
	pub async fn new(
		...
		Ok(Self { inner, next_write_version, handle })
	}
