...
impl StoreFixture {
	pub fn new(backend: Backend, benchmark_name: &str) -> Self {
		...
		let store: Arc<DynStore> = match backend {
			...
			#[cfg(feature = "postgres")]
			Backend::Postgres => {
				let connection_string = std::env::var("LDK_NODE_POSTGRES_BENCH_URL")
					.unwrap_or_else(|_| {
						panic!(
							"LDK_NODE_POSTGRES_BENCH_URL must be set to run postgres database benchmarks"
							)
					});
				let table_name = postgres_table_name(&store_id);
				let store = runtime
					.block_on(crate::io::postgres_store::PostgresStore::new(
	...
	pub fn write_payment_batch(&self, payments: Vec<PaymentDetails>) {
		self.runtime.block_on(write_payment_batch(&*self.store, payments));
	}

	pub fn write_payment_batch_from_offset(&self, offset: u64) {
		self.write_payment_batch(payment_details_batch(offset));
	}

	pub fn write_payment(&self, idx: u64) {
		self.runtime.block_on(write_payment(&*self.store, payment_details(idx)));
	}

	pub fn write_payment_update(&self, idx: u64) {
		self.runtime.block_on(write_payment_update(&*self.store, payment_update(idx)));
	}

	pub fn write_payment_update_batch(&self, updates: PaymentUpdateBatch) {
		self.runtime.block_on(write_payment_update_batch(&*self.store, updates.0));
	}

	pub fn read_payment(&self, idx: u64) -> Vec<u8> {
		self.runtime.block_on(read_payment(&*self.store, payment_key(idx))).unwrap()
	}

	pub fn remove_payment_key(&self, key: &str) {
		self.runtime.block_on(remove_payment(&*self.store, key));
	}

	pub fn reload_payments(&self) -> Vec<PaymentDetails> {
		self.runtime
			.block_on(read_all_objects::<PaymentDetails, _>(
				&*self.store,
	...
	pub fn write_payment_batch_concurrent(&self, offset: u64, same_key: bool) {
		self.runtime.block_on(write_payment_batch_concurrent(
			DynStoreRef(Arc::clone(&self.store)),
			offset,
			same_key,
		));
	}

	pub fn write_pending_payment_batch(&self, payments: PendingPaymentBatch) {
		self.runtime.block_on(write_pending_payment_batch(&*self.store, payments.0));
	}

	pub fn write_pending_payment_update_batch(&self, updates: PendingPaymentUpdateBatch) {
		self.runtime.block_on(write_pending_payment_update_batch(&*self.store, updates.0));
	}

	pub fn insert_update_read_payment(&self, idx: u64) -> Vec<u8> {
...
impl Drop for StoreFixture {
	fn drop(&mut self) {
		...
		if let Some(cleanup) = self.postgres_cleanup.take() {
			self.runtime.block_on(cleanup.drop_table());
			return;
		}
...
impl PaginatedStoreFixture {
	pub fn new(backend: Backend, benchmark_name: &str) -> Option<Self> {
		let store_id = next_store_id();
		let storage_path = bench_storage_path(backend, benchmark_name, &store_id);
		let runtime = tokio::runtime::Builder::new_multi_thread()
			.worker_threads(2)
		...
		match backend {
			Backend::Filesystem => {
				let store = FilesystemStoreV2::new(storage_path.clone()).unwrap();
				runtime.block_on(populate_async_store(&store));
				Some(Self {
					inner: PaginatedStoreFixtureInner::Filesystem { store, runtime, storage_path },
				})
			},
			Backend::Sqlite => {
				let store = SqliteStore::new(storage_path.clone(), None, None).unwrap();
				runtime.block_on(populate_async_store(&store));
				Some(Self {
					inner: PaginatedStoreFixtureInner::Sqlite { store, runtime, storage_path },
				})
			},
			#[cfg(feature = "postgres")]
			Backend::Postgres => {
				...
				Some(Self {
					inner: PaginatedStoreFixtureInner::Postgres { store, runtime, cleanup },
				})
			},
		}
	}

	pub fn list_first_page(&self) -> usize {
		self.runtime().block_on(self.list_first_page_async())
	}

	pub fn list_second_page(&self) -> usize {
		self.runtime().block_on(self.list_second_page_async())
	}

	async fn list_first_page_async(&self) -> usize {
		match &self.inner {
	...
	async fn list_second_page_async(&self) -> usize {
		match &self.inner {
	...
	fn runtime(&self) -> &tokio::runtime::Runtime {
		match &self.inner {
			PaginatedStoreFixtureInner::Filesystem { runtime, .. } => runtime,
			PaginatedStoreFixtureInner::Sqlite { runtime, .. } => runtime,
			#[cfg(feature = "postgres")]
			PaginatedStoreFixtureInner::Postgres { runtime, .. } => runtime,
		}
	}
}
...
pub fn configured_backends() -> Vec<Backend> {
	#[cfg(feature = "postgres")]
	{
		...
		if std::env::var("LDK_NODE_POSTGRES_BENCH_URL").is_ok() {
